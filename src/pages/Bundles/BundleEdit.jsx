...
const BundleEdit = () => {
  ...
  const [form, setForm] = useState(null); // null trong lúc loading, khác Create luôn có sẵn form
  ...
  // Tải dữ liệu combo hiện có — endpoint GET /Bundles/{id} đã trả sẵn Items kèm đủ
  // productName/productImage/productPrice (tính từ giá THẬT hiện tại), nên map thẳng
  // sang state `selected` mà không cần gọi thêm request nào khác.
  useEffect(() => {
    api.get(`/Bundles/${id}`)
      .then(res => {
        const b = res.data;
  ...
  const finalPrice = form ? Math.round(subtotal * (1 - (Number(form.discountPercent) || 0) / 100)) : 0;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Lỗi khi cập nhật combo! Kiểm tra lại backend.");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (!form) return (
    <div className="bundle-form-page">
      <div className="form-error">⚠️ {error || "Không tìm thấy combo!"}</div>
